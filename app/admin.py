...
def create_house():
    ...
    db = get_db()
    db.execute('''
        INSERT INTO houses (title, description, address, price, area, rooms, house_type,
    ...
    ''', (
        ...
        data.get('house_type'), data.get('floor_info'), data.get('orientation'),
        data.get('decoration'),
...
def update_house(house_id):
    ...
    if 'facilities' in data:
        ...
        params.append(json.dumps(data['facilities'], ensure_ascii=False))
    if 'images' in data:
    ...
    if fields:
        ...
        db.execute(f'UPDATE houses SET {", ".join(fields)} WHERE id = ?', params)
        db.commit()
...
def delete_house(house_id):
    db = get_db()
    db.execute('DELETE FROM appointments WHERE house_id = ?', (house_id,))
    db.execute('DELETE FROM houses WHERE id = ?', (house_id,))
    db.commit()
    return jsonify({'message': '房源已删除'})


@admin_bp.route('/api/all-houses', methods=['GET'])
@admin_required
def all_houses():
    db = get_db()
    houses = db.execute('SELECT * FROM houses ORDER BY created_at DESC').fetchall()
    result = []
    for h in houses:
        house = dict(h)
...
def update_appointment_status(apt_id):
    ...
    db = get_db()
    db.execute('UPDATE appointments SET status = ? WHERE id = ?', (status, apt_id))
    db.commit()
